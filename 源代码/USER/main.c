...
{
		 //CCS811  CO数据采集
	   ON_CS(); 					   	//nWAKE pin is asserted at least 50μs before the transaction and kept asserted throughout,nWAKE pin is active low     
		 Single_ReadI2C(CCS811_Add,0x00,&Status,1);
	   Single_ReadI2C(CCS811_Add,0xE0,&Error_ID,1);
	   Single_ReadI2C(CCS811_Add,0x02,BUUF,8);
...
void control_set(void)
{
	switch(control_mode)
		...
		if(SUR_status.light<SUR_status.light_num)  LED=1;  ///光照不够开灯
			...
			break;
		case 1://手动
		if(FAN_KEY) FAN=1;  //手动开风扇
		else FAN=0;
		if(Water_pump_KEY)  Water_pump=1;  ///手动开水泵
		else Water_pump=0;
		if(LED_KEY)  LED=1;  ///手动开灯 
		else LED=0;
			break;
		
		case 2://云端
		if(FAN_onenet) FAN=1;  //云端开风扇
		else FAN=0;
		if(Water_pump_onenet)  Water_pump=1;  //云端开水泵
		else Water_pump=0;
		if(LED_onenet)  LED=1;  //云端开开灯
		else LED=0;
			break;
		default:break;
	}	
 ...
 void Num_Test(void)  //手动控制显示
{
	...
	if( Water_pump_KEY)
	Gui_DrawFont_GBK16(60,96,BLACK,WHITE,"开");
	Gui_DrawFont_GBK16(60,96,BLACK,WHITE,"关");
	if(LED_KEY)
	Gui_DrawFont_GBK16(60,112,BLACK,WHITE,"开");
	Gui_DrawFont_GBK16(60,112,BLACK,WHITE,"关");
	if(FAN_KEY)
	Gui_DrawFont_GBK16(60,128,BLACK,WHITE,"开");
	Gui_DrawFont_GBK16(60,128,BLACK,WHITE,"关");	
}
